...
- ✅ Issue05: カスタムストレージロケーション機能 (2025-04-21完了)
  - ✅ フェーズ1: バックエンド拡張 (2025-04-20完了)
  - ✅ フェーズ2: フロントエンド拡張 (2025-04-20完了)
  - ✅ フェーズ3: データ移行と互換性 (2025-04-21完了)
  - ✅ フェーズ4: テストとドキュメント (2025-04-21完了)
...
│   │   │   └── Button.tsx
│   │   │   └── Button.test.tsx  # テストファイルはソースファイルの隣に配置
│   │   ├── hooks/      # カスタムフック
│   │   ├── services/   # APIクライアント
│   │   ├── types/      # 型定義
│   │   └── ...
...
フロントエンドとバックエンドでは、異なるテストファイル配置方針を採用しています：

- **フロントエンド（client）**: テストファイルをソースファイルの隣に配置（例: `Button.tsx` と `Button.test.tsx`）
  - React/TypeScriptのエコシステムで一般的なアプローチ
  - コンポーネントとそのテストの関連性が明確
  - コンポーネント指向の開発に適している

- **バックエンド（server）**: 専用のテストディレクトリを使用（`tests/unit/`, `tests/integration/`など）
  - Node.js/Expressのエコシステムで一般的なアプローチ
  - テストの種類（単体テスト、統合テスト）による明確な分離
  - 将来的にリポジトリを分割する可能性を考慮

この方針は、それぞれの技術スタックに最適化されたアプローチを採用し、将来的なメンテナンス性を高めることを目的としています。
