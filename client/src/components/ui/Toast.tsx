...
export function Toast({ message, type, onClose, duration = 5000 }: ToastProps) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 max-w-md">
      <div
        className={`rounded-lg shadow-lg p-4 flex items-start ${typeClasses[type]}`}
        role="alert"
      >
        ...
        <button
          type="button"
          className="ml-4 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300"
          onClick={onClose}
          aria-label="閉じる"
        >
          <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
